...
def compute_speed(df_DLC, x_col, y_col, speed_col, time_col = 'time_seconds'):

    # compute speed
    distance = np.sqrt(np.diff(df_DLC[x_col])**2 + np.diff(df_DLC[y_col])**2)
    time = np.diff(df_DLC[time_col])
    speed = distance/time
...
def compute_head_angle_to_pup(df_DLC, add_vector_columns = False,
                              earRight_x_col = 'earRight_x', earRight_y_col = 'earRight_y',
                              earLeft_x_col = 'earLeft_x', earLeft_y_col = 'earLeft_y',
                              nose_x_col = 'nose_x', nose_y_col = 'nose_y',
                              pup_x_col = 'pup_x', pup_y_col = 'pup_y',
                              head_angle_to_pup_col = 'head_angle_to_pup_degrees'):

    # define mouse head direction with respect to average of ears and nose
    earRight_x, earRight_y, earLeft_x, earLeft_y,  = df_DLC[earRight_x_col], df_DLC[earRight_y_col], df_DLC[earLeft_x_col], df_DLC[earLeft_y_col]
    nose_x, nose_y, pup_x, pup_y = df_DLC[nose_x_col], df_DLC[nose_y_col], df_DLC[pup_x_col], df_DLC[pup_y_col]
...
def extract_base_parameters(df_DLC, df_summary,
                            trial_end_col = 'BehavRecdTrialEndSecs',
                            pup_drop_col = 'PupDispDropSecs',
                            trial_num_col = 'TrialNum',
                            frame_index_col = 'frame_index',
                            pup_speed_col = 'pup_speed_px/s',
                            mouse_speed_col = 'ms_speed_px/s',
                            distance_col_mouse_pup = 'distance_mouse_to_pup',
                            distance_col_head_pup = 'distance_head_to_pup',
                            earRight_x_col = 'earRight_x', earRight_y_col = 'earRight_y',
                            earLeft_x_col = 'earLeft_x', earLeft_y_col = 'earLeft_y',
                            head_x_col = 'endHeadbar_x', head_y_col = 'endHeadbar_y',
                            nose_x_col = 'nose_x', nose_y_col = 'nose_y',
                            pup_x_col = 'pup_x', pup_y_col = 'pup_y',
                            msTop_x_col = 'msTop_x', msTop_y_col = 'msTop_y',
                            head_angle_to_pup_col = 'head_angle_to_pup_degrees'):
    ...
    end_times, start_times, trial_nums = df_summary[trial_end_col], df_summary[pup_drop_col], df_summary[trial_num_col]
    ...
    for end, start, trial_num in zip(end_times, start_times, trial_nums):
            ...
            # compute speed
            trial_DLC = compute_speed(trial_DLC, x_col = msTop_x_col, y_col = msTop_y_col, speed_col = mouse_speed_col)
            trial_DLC = compute_speed(trial_DLC, x_col = pup_x_col, y_col = pup_y_col, speed_col = pup_speed_col)

            # compute distance to pup
            trial_DLC = compute_distance_to_pup(trial_DLC, x_col = msTop_x_col, y_col = msTop_x_col,
                                                pup_x_col = pup_x_col, pup_y_col = pup_y_col,
                                                distance_col = distance_col_mouse_pup)
            
            trial_DLC = compute_distance_to_pup(trial_DLC, x_col = head_x_col, y_col = head_y_col,
                                                pup_x_col = pup_x_col, pup_y_col = pup_y_col,
                                                distance_col = distance_col_head_pup)
            
            # compute head angle to pup
            trial_DLC = compute_head_angle_to_pup(trial_DLC, add_vector_columns = False,
                                                  earRight_x_col = earRight_x_col, earRight_y_col = earRight_y_col,
                                                  earLeft_x_col = earLeft_x_col, earLeft_y_col = earLeft_y_col,
                                                  nose_x_col = nose_x_col, nose_y_col = nose_y_col,
                                                  pup_x_col = pup_x_col, pup_y_col = pup_y_col,
